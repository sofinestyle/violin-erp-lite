...
describe("Master Data PC pages", () => {
  ...
  it("renders product category presets as editable suggestions", () => {
    const category = MASTER_WORKBENCHES.find(
      (definition) => definition.key === "product-categories",
    );
    const categoryName = category?.fields.find((field) => field.key === "categoryName");
    expect(categoryName?.inputMode).toBe("datalist");
  ...
  it.each(["product-categories", "brands", "ecommerce-platforms", "stores"])(
    "hides Phase 2 code inputs and preserves code list columns for %s",
    (resource) => {
      const definition = MASTER_WORKBENCHES.find((item) => item.key === resource)!;
      const code = definition.fields.find((item) => item.key === definition.codeField)!;
      expect(code.autoGeneratedCode).toBe(true);
      expect(code.required).toBe(false);
      const html = renderToStaticMarkup(
        <PermissionProvider roleCodes={["administrator"]} permissions={[]}>
          <MasterDataWorkbench definition={definition} group="master" />
        </PermissionProvider>,
      );
      expect(html).not.toContain(`name="${definition.codeField}"`);
      expect(html).toContain(code.label);
      for (const selected of [null, { id: "fixture", [definition.codeField]: "LEGACY-001" }]) {
        ...
        expect(fieldHtml).not.toContain("<input");
        expect(fieldHtml).toContain(selected ? "LEGACY-001" : "保存后由系统自动生成");
      }
    },
  );
