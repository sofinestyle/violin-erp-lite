...
function shouldSubmitField(
  ...
  if (field.autoGeneratedCode) return false;
  if (!fieldConditionMatches(field, selected, formValues)) return false;
  return !(
    selected && ["password", "roleAssignments", "roleCode", "isSystemRole"].includes(field.key)
  );
...
export function MasterDataFieldControl({
  ...
  if (field.autoGeneratedCode) {
    return (
      <div className={fieldColumnClass(field)}>
        <span className="text-sm font-medium">{field.label}</span>
        <div className="rounded-md border border-dashed bg-[#F9FAFB] px-3 py-2 text-sm text-[#374151]">
          {selected ? displayValue(selected[field.key]) : "保存后由系统自动生成"}
        </div>
        {field.helpText ? (
