...
当前状态：API Master Specification v1.1 为 Completed / Approved / Frozen，正式接口总数为 315。API Change Request 001 已正式批准，库存盘点、销售退货和报损的 43 个接口已纳入冻结基线；v1.0 与原 272 个接口保留为历史冻结基线。Phase 6 为 Completed / Approved / Frozen，Phase 6 Final Consistency Review 为 Completed / Approved。Phase 7 为 In Progress，Task 7.2 为 In Progress；当前正式任务为 Task 7.2。
...
12. 禁止通过 Phase 6 文档、页面代码或实现代码绕过本规范；后续修改必须经过正式 DCR 或 Change Request；Phase 6 为 Completed / Approved / Frozen，Phase 6 Final Consistency Review 为 Completed / Approved；Phase 7 为 In Progress，Task 7.2 为 In Progress；当前正式任务为 Task 7.2。
