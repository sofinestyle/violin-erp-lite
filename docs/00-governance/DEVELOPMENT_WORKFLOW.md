...
5. Task 3.5 字段类型、约束与索引设计（Field Type, Constraint and Index Design）：In Progress。

Task 3.5 内部小任务进度：

1. Task 3.5.1 字段数据类型规范（Field Type Standard）：Completed / Approved；
2. Task 3.5.2 主键与唯一约束设计（Primary Key and Unique Constraint Design）：Not Started。

Phase 3 正在进行，Task 3.5 状态为 In Progress。Task 3.5.1 已建立数据库无关的逻辑类型、UUID 原则、字符串长度、数值精度、日期时间、ENUM、JSON、敏感字段、文件字段和只追加表类型规范。本小任务不定义具体主键或唯一约束清单、外键删除策略、索引、Check 约束、SQL、ORM、Schema、Migration 或数据库选型；Task 3.5.1 验收通过前不得启动 Task 3.5.2。
