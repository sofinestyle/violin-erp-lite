...
version: 1.2
status: In Progress / Database Approved / API Pending Approval
...
> 项目负责人已批准 Database Change Request 002 及其认证会话持久化 Completion Fix；API Change Request 002 仍为 Proposed / Pending Approval。本文件整体尚未完成，不授权认证业务开发。
...
Database Change Request 002 及其 Completion Fix 已批准并同步为 Frozen Database Logical Design v2.0，正式覆盖微信身份映射、Refresh Token 轮换、重放识别和登出撤销的持久化；API Change Request 002 仍待批准。因此 Batch 7.6-B 的认证实现部分继续暂停。
...
| AUTH-SSOT-003 | API Master Specification v1.1；Task 5.5 | `SEC-002` 至 `SEC-005` 统一处理刷新、登出、会话和权限 | v2.0 Completion Fix 已补齐会话持久化；兼容 DTO、客户端摘要及解绑边界仍未批准 | 数据库阻塞已关闭，API 契约仍阻塞实现 |
...
这是既有 Approved/Frozen 输入之间的可实现性缺口。数据库部分已经正式关闭；API DTO、错误码和会话兼容部分仍未批准。
...
后三项 API / Phase 6 正式规格仍待 API Change Request 002 批准，本轮未修改。
...
- AppID/Secret 环境变量和认证使用方式留待 API Change Request 002 获批后实现。

## 10. 兼容性影响

- PC 继续使用同一路径；客户端需按批准后的判别式 DTO 提交 `loginType: "password"`，不存在路径迁移；
- Mini Program 从当前空会话壳层接入 `wechat-bind` / `wechat`；
- 现有 Access Token 与 Refresh Token 技术可复用；数据库已持久化轮换、重放和撤销事实，但正式请求 DTO、响应和 Service 边界仍必须由 API Change Request 固化；
- `X-Client-Type` 继续只用于兼容、审计和监控，不参与身份或授权决定；
- 错误码复用 Frozen `AUTH_CREDENTIAL_INVALID`、`AUTH_REFRESH_TOKEN_INVALID`；不得重复引入用户指令中的同义名称；
- Database Logical Design 已由 v1.1 升至 v2.0；API Master Specification v1.2 建议仍待项目负责人批准。
...
- API Change Request 002 批准、API/Phase 6 SSOT 同步并完成 Frozen Consistency Review 与 GitHub 技术验收后，方可重新启动 Batch 7.6-B；
- Phase 7 Final Consistency Review 与 Phase 8 均不启动。

## 12. 剩余批准前禁止事项

- 不修改 `API_SPEC.md`、Phase 5 或 Phase 6 正式规格；
...
- 不将尚未批准的 API 方案描述为 Approved / Frozen。
...
1. 是否批准 `password`、`wechat-bind`、`wechat` 三种 `loginType` 共用 `SEC-001`；
2. 是否批准首次绑定在单一事务中完成账号校验和绑定，并在提交后签发令牌；
3. 是否批准本次不新增绑定、解绑或查询 API；管理能力未来另行变更；
4. 是否批准 `mustChangePassword = true` 时拒绝微信绑定，并新增 `AUTH_PASSWORD_CHANGE_REQUIRED`；
5. 是否批准 API Master Specification 目标版本 v1.2；
6. 对已解绑身份是否允许经管理员批准绑定到另一系统账号，以及相应冷却期/人工复核策略；
7. 未知用户登录失败安全事件是否需要另行提出安全事件持久化 DCR。

## 15. 提案结论

方案 A 的数据库部分及认证会话持久化 Completion Fix 已正式批准：Database Logical Design v2.0 以最小微信身份映射表和最小认证会话表保留 `users` 为唯一用户身份。现有 `SEC-001` 判别式 DTO、错误码及会话兼容方案仍待 API Change Request 002 批准；在 API SSOT 更新、Frozen Consistency Review 和 GitHub 技术验收完成前不得开始认证开发。
...
本次只关闭认证数据库持久化缺口。API Change Request 002 仍为 Proposed / Pending Approval，本文件整体仍为 In Progress / Database Approved / API Pending Approval，Batch 7.6-B 继续暂停。
