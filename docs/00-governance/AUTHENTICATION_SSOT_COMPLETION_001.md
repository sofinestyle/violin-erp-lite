...
> 项目负责人已批准本文件的 Database Change Request 002 部分；API Change Request 002 仍为 Proposed / Pending Approval。本文件整体尚未完成，不授权认证业务开发。
...
Database Change Request 002 已批准并同步为 Frozen Database Logical Design v2.0；API Change Request 002 仍待批准。因此 Batch 7.6-B 的认证实现部分继续暂停。
...
Frozen Database Logical Design v2.0 已正式新增 `user_wechat_identities`，只承担外部微信身份到现有 `users.id` 的映射，不承担用户、角色、权限、昵称或头像管理。
...
完整字段、约束、Migration 和正式计数见已批准的 `DATABASE_CHANGE_REQUEST_002.md`。
...
- Prisma Schema、Migration、Database SSOT 和 `prisma/mapping-audit.json` 已在同一批准批次保持一致；
...
方案 A 的数据库部分已正式批准：Database Logical Design v2.0 新增最小微信身份映射表并保留 `users` 为唯一用户身份。现有 `SEC-001` 判别式 DTO、错误码及会话兼容方案仍待 API Change Request 002 批准；在 API SSOT 更新、Frozen Consistency Review 和 GitHub 技术验收完成前不得开始认证开发。
