...
Task 3.5 已完成并获得批准（Completed / Approved）。Task 3.5.1 已批准数据库无关的逻辑字段类型规范：主键及对象引用统一采用 UUID 并优先 UUID v7，数量和金额采用 `DECIMAL(18,4)`，税率和百分比采用 `DECIMAL(7,4)`，纯业务日期与准确操作时间分别采用 `DATE` 和 `DATETIME`。同时明确 ENUM 稳定代码、JSON 使用边界、敏感字段、文件字段及只追加表类型原则。
