...
`CBR-001` 支持单号、发货批次、目的国家、来源/在途/海外仓、生产单、SKU、`shipmentStatus`、通用/审核状态、发运/预计/实际到达日期、承运商、物流单号、运输方式、是否逾期及是否有差异。

Frozen 跨境发货单没有平台或店铺字段，因此发货列表不提供平台/店铺筛选。平台或店铺仅可在有正式 `import_tasks.store_id` 的导入结果查询中使用。厂家通过可选生产单派生，不作为发货单新增字段。

## 19. 创建跨境发货单

`CBR-003` 请求：`documentDate`、可选 `productionOrderId`、`sourceWarehouseId`、`transitWarehouseId`、`destinationWarehouseId`、`shipmentBatchNo`、`carrierName`、`trackingNo`、`transportMethod`、`departureDate`、`estimatedArrivalDate`、`destinationCountry`、`remark`、`items`。明细为可选 `productionOrderItemId`、`skuId`、`quantity`、`batchNo`、`unitCost`、`remark`。
...
`shipmentStatus` 只使用待发货、部分发货、已全部发货、已取消、发货异常。发运后禁止普通修改关键仓库、生产来源、SKU、批次和数量。由于第 32 节 Frozen 缺口，本 Task 不定义物流、海外收货和差异状态写接口，也不定义手工海外收货确认接口。
...
发运确认不得直接增加海外仓库存。海外仓正式库存变化只能由 Task 5.5 正式 Excel 导入执行及其已批准匹配/收货事务触发；本 Task 仅查询执行结果。网络重试不得形成重复流水或重复累计。
...
当前最新余额可作为“最新视图”，但 Frozen 没有海外库存历史快照对象；历史只能查询导入任务/明细及库存流水，不能返回不存在的历史余额快照。正式上传、校验、确认、匹配写入和执行接口留待 Task 5.5。
...
客户端不得直接提交任意目标状态。物流、海外收货和差异状态动作因第 32 节 Frozen 字段缺口停止设计。
...
```json
{"documentDate":"2026-07-21","productionOrderId":"018f...501","sourceWarehouseId":"018f...502","transitWarehouseId":"018f...503","destinationWarehouseId":"018f...504","shipmentBatchNo":"CB20260721-01","carrierName":"Carrier A","trackingNo":"TRACK-001","transportMethod":"sea","departureDate":"2026-07-22","estimatedArrivalDate":"2026-08-20","destinationCountry":"US","items":[{"productionOrderItemId":"018f...511","skuId":"018f...512","quantity":"12.0000","batchNo":"B202607","unitCost":"300.0000"}]}
```
...
| Task 4.10 手工海外收货、物流/收货/差异状态写入 | 无正式接口 | 第 32 节停止 | Frozen 字段不足且本任务限定 Excel 来源 |
| 历史海外库存余额快照/差异快照 | 无正式接口 | 第 32 节停止 | Frozen 无快照对象；可查导入历史和流水 |

除第 32 节登记项外，Task 4.8 当前库存入口、Task 4.9 本 Task 范围页面和 Task 4.10 可映射操作均已有接口或明确留待 Task 5.5。
...
| 冲突位置 | Frozen/Approved 依据 | 缺失内容 | 本 Task 处理 | 建议 |
| --- | --- | --- | --- | --- |
| 跨境物流、海外收货及差异状态写接口 | Task 2.5 第 10 节、Task 4.10 第 3.7 节要求四个独立状态维度；Task 3.4 `cross_border_shipments` 只有通用 `status`、`approval_status` 和单一 `shipment_status` | 独立物流状态、海外收货状态、差异处理状态字段及完整持久化映射 | 只设计通用审核与发货执行动作；停止物流、手工收货及差异状态写接口 | 如必须持久化四维状态，发起正式 DCR；否则由项目负责人批准派生/导入口径 |
| 海外库存历史余额快照与快照差异 | Frozen 只有当前 `inventories`、只追加流水及导入任务/明细，没有海外库存快照表或快照日期字段 | 可按数据日期还原的正式历史余额快照对象 | 只提供当前余额、流水、导入历史和来源追溯；不登记虚假快照接口 | 如历史余额快照为必需能力，发起正式 DCR；否则调整页面/API 为导入历史与流水查询 |
| Task 4.10 手工海外收货入口与本 Task Excel 唯一来源 | Approved 页面允许受控手工登记；本次正式指令要求海外仓库存来自正式 Excel 导入结果，且导入写接口归 Task 5.5 | 统一的海外收货写入口决策 | 本 Task 不设计手工收货或海外库存增加接口；只读取 Task 5.5 结果 | 由项目负责人形成正式页面/API 口径决定；若无需新增结构则不必 DCR |

以上冲突部分已停止，未自行修改 Frozen 数据库或 Approved 页面。前两项在坚持原能力时需要 Database Change Request；第三项需要正式页面/API 口径决定。

### 32.2 正式结论

1. Task 5.1、Task 5.2、Task 5.3 均为 Completed / Approved；
2. Task 5.4 已完成可映射范围设计，状态为 Completed / Pending Approval；
...
12. 当前下一步为 Task 5.4 GitHub 验收，不得开始 Task 5.5；
