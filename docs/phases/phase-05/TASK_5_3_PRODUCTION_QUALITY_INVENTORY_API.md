...
| 分批完工 | Task 2.5 第 6 节 | `production_completion_records`、`production_completion_record_items`；生产单、日期、目标仓、数量、原生产明细 | Task 4.7 第 8 节 | 完工登记及目标仓库权限 |
...
分批完工创建、提交和确认因第 30 节 Frozen 映射缺口停止设计，不计入正式接口。生产正式接口数量为 25 个。
...
库存正式接口数量为 26 个。Task 5.3 正式接口总数为 25 + 10 + 26 = 61 个。
...
`PRO-021` 和 `PRO-022` 查询生产单、完工批次号、完工日期、目标仓、主记录数量、现有 `completionStatus`、明细原生产单行、SKU 快照、完工数量、业务批次号及备注。

创建时本应校验：目标仓为授权且适用的公司仓或对应厂家仓；每行只引用所属生产单明细；主数量等于明细合计；每行及累计完工量不超计划；同一生产单批次号唯一；同一完工记录同一生产明细只出现一次；重复请求幂等；并发以生产单版本、累计事实和唯一约束保护。

但 Frozen 文档没有定义 `completion_status` 的允许值及提交/确认流转，因此本 Task 不设计创建、提交或确认写接口，不把分批完工直接视为验收或入库。详见第 30 节。
...
`INS-003` 请求：`sourceType`、适用的 `purchaseOrderId` 或 `productionOrderId`、`inspectionDate`、`inspectionWarehouseId`、`inspectorId`、`remark`、`items`。明细为 `sourceItemId`、`skuId`、`inspectedQuantity`、`qualifiedQuantity`、`unqualifiedQuantity`、`pendingQuantity`、缺陷信息、结果、处置及备注。
...
每行必须满足 `inspectedQuantity = qualifiedQuantity + unqualifiedQuantity + pendingQuantity`。主单 `totalInspectedQuantity`、`totalQualifiedQuantity` 和 `totalUnqualifiedQuantity` 分别按同名明细语义汇总。累计验收不得超过来源合法可验收量，重复验收以来源明细累计事实和条件更新防护。

当任一明细 `pendingQuantity > 0` 时，主表缺少待处理总量字段且无法同时满足 Frozen 主表平衡约束；该写入分支返回 `DESIGN_FROZEN_MAPPING_MISSING` 并停止，不能把待处理量擅自并入不合格总量。详见第 30 节。
...
确认原子写入已确认状态、状态历史、审计日志及必要审批事实。只有已确认的合格数量获得后续正式入库资格；不合格和待处理量不得进入正常可用库存。存在待处理量时按第 13、30 节停止确认。确认不得创建入库单、库存余额或库存流水。
...
Frozen 只定义 `completion_status` 字段，未定义状态值和转换矩阵。创建、提交和确认动作停止设计，不得由客户端提交目标状态。
...
创建生产单、所有状态动作、开始生产、新增进度、创建验收、验收确认、创建调整单、执行调整、创建生产付款和导出均要求 `Idempotency-Key`。普通编辑使用 `versionNo` 或 `updatedAt` 条件更新。
...
生产单创建、修改、状态动作、开始生产、进度、完工查询异常、生产付款、验收创建/修改/确认/撤销/作废、预警处理、库存调整、库存事务、敏感金额查看、导出和高风险库存查询均写 `audit_logs`。状态变化写 `document_status_histories`，审核动作写 `approval_records`。
...
| 分批完工 | `PRO-021`、`PRO-022` | 只读 | 写接口因第 30 节停止 |
...
Task 4.7 页面中的“来源完工批次”和分批完工写操作无法完整映射 Frozen 关系/状态，已在第 30 节登记。其他 Approved 页面操作均有对应接口或明确留待 Task 5.4/Task 5.5。
...
| 冲突位置 | Frozen/Approved 依据 | 缺失内容 | 本 Task 处理 | 后续选择 |
| --- | --- | --- | --- | --- |
| 分批完工创建、提交、确认 | Task 3.4 第 211 至 217 行规定 `completion_status` 必填；Task 2.5 第 175 至 246 行及 Task 4.7 第 350 至 387 行均未定义完工记录允许值及转换 | 完工记录状态值和正式状态机 | 只设计列表/详情；停止三个写动作，不虚构状态 | 项目负责人可调整为原子登记口径并批准状态语义，或发起 DCR/正式变更 |
| 验收单直接关联完工批次 | Task 3.4 第 221 至 230 行只允许采购/生产互斥来源和对应订单明细；Task 4.7 第 389 至 407 行要求来源完工批次 | 验收单/明细到完工记录/明细的正式关系 | 验收仅关联生产单及生产明细；不接收或返回持久化完工批次 ID | 优先调整页面/API 为派生展示；如必须持久追溯批次则发起 DCR |
| 验收待处理数量汇总 | Task 3.4 第 221 至 230 行仅在明细提供 `pending_quantity`；Task 3.5.5 第 223 至 226 行同时要求主表“验收总量＝合格总量＋不合格总量”和明细“验收量＝合格＋不合格＋待处理” | 主表待处理总量或经正式批准的汇总语义 | 待处理量大于零的创建、提交和确认分支停止；不把待处理量并入不合格量 | 项目负责人明确主表汇总语义；如需新增字段或改约束则发起 DCR |

本 Task 未发起或执行 DCR，未修改 Frozen 数据库。上述部分已停止设计，需项目负责人决定是否通过口径调整关闭，或正式发起 DCR；其余接口不受影响。

### 30.2 正式结论

1. Task 5.1、Task 5.2 均为 Completed / Approved；
2. Task 5.3 可映射范围设计已完成，状态为 Completed / Pending Approval；
3. 共定义 61 个正式接口：生产 25 个、质量验收 10 个、库存 26 个；
4. 生产与采购保持平行，生产单不保存统一目标仓库；
5. 验收确认只形成入库资格，不直接修改库存；
6. 库存余额只能由正式库存事务改变，流水只追加；
7. 库存调整执行使用原子事务并防止负库存；
8. 三个 Frozen 映射缺口已登记并停止冲突部分设计；
9. 未新增字段、表、状态、关系、索引或业务对象；
10. 未创建真实 API Route，未编写业务代码，未修改 Frozen 数据库或 Approved 页面；
11. Phase 5 保持 In Progress，Task 5.4 和 Task 5.5 保持 Waiting；
12. 当前下一步为 Task 5.3 GitHub 验收，不得开始 Task 5.4；
