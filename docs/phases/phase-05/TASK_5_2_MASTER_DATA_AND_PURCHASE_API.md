...
接口数量经冲突修正后重新核算：九类正式基础资料资源各 8 个通用接口，共 72 个；客户快照只读接口 2 个，基础资料合计 74 个；采购管理接口 `PUR-001` 至 `PUR-029` 共 29 个；总计 72 + 2 + 29 = 103 个。原冲突能力从未登记为正式接口，因此修正后接口总数保持 103 个。
...
- 采购订单正式状态仅使用草稿、待审核、已审核、已驳回、已完成、已取消和已作废；已完成由系统按业务完成条件判定。
...
采购执行进度只汇总采购订单及其合法关联的验收、入库、付款和退货记录。`PUR-013` 和 `PUR-014` 只查询正式验收和入库记录，不包含写接口。
...
Task 4.5 与 Task 4.6 已按项目负责人决定完成页面口径修正；页面与本 Task 的 Frozen 对象、字段、关系和状态映射现已闭合，详见第 26 节。
...
| 原冲突位置 | 项目负责人决定 | 页面修正结果 | API 结果 | 状态 |
| --- | --- | --- | --- | --- |
| 可编辑计量单位 | 不建立可维护对象，不修改 Frozen 数据库 | Task 4.5 改为产品和 SKU 表单中的固定受控值选择，不提供独立页面或维护能力 | 沿用产品 `defaultUnit` 与 SKU `unit` 字段，不新增计量单位资源或接口 | 已解决 / Closed |
| 采购类型、采购负责人 | 不新增字段，以现有责任留痕表达 | Task 4.6 删除对应字段、列表列和筛选；责任通过创建人、提交人、审核人及审计记录追溯 | 采购接口不接收、不返回、不筛选对应字段 | 已解决 / Closed |
| 独立到货登记 | 不新增到货对象 | Task 4.6 将入口修正为创建采购验收单和查看到货进度 | 保留 `PUR-012` 进度查询及 `PUR-013` 关联验收查询，不新增到货写接口 | 已解决 / Closed |
| 采购与生产关系 | 保持采购与委外生产平行 | Task 4.6 不提供采购厂家筛选、生产任务直达或已生产数量 | 采购接口只映射供应商及合法采购记录，不新增采购到生产关系 | 已解决 / Closed |
| 采购关闭状态 | 不新增状态，完成由系统判定 | Task 4.6 删除关闭按钮、状态和筛选，保留七个正式状态 | 保留提交、审核、驳回、反审核、取消、作废及系统完成判定，不新增关闭接口 | 已解决 / Closed |

项目负责人决定通过调整 Approved 页面口径关闭上述冲突，不发起 Database Change Request。Database Logical Design v1.0 保持 Frozen 且未修改；所有原冲突均已解决，不再阻塞 Task 5.2 批准。

### 26.2 正式结论

1. Task 5.1 状态为 Completed / Approved，并作为本文件统一规范；
2. Task 5.2 基础资料与采购管理 API 设计已完成并获得批准，状态为 Completed / Approved；
3. Phase 5 保持 In Progress；Task 5.3 至 Task 5.5 均为 Completed / Approved；
4. 经逐项重新核算，共定义 103 个接口：九类基础资料通用接口 72 个、客户快照只读接口 2 个、采购管理接口 29 个；
5. 本次设计反审核接口，依据 Task 2.5 第 35、79 至 84 行，且严格限制无库存流水和无下游关联；
6. 第 26.1 节原冲突已通过 Approved 页面口径修正全部关闭，未发起 DCR，未新增任何字段、表、状态、唯一约束、索引、关系或业务对象；
7. 未创建真实 API Route，未编写业务代码，未修改 Frozen 数据库；Task 4.5 与 Task 4.6 仅按项目负责人决定修正页面口径；
8. 技术开发保持 Not Started；
9. 当前下一步为 Phase 5 Final Consistency Review GitHub 验收；本任务不开始 Phase 6。
