...
| 下一任务 | Task 3.4：字段结构设计（Not Started） |
...
| 序号 | 表名 | 中文名称 | 表类别 | 用途 |
| --- | --- | --- | --- | --- |
| 22 | `inspection_orders` | 质量验收单主表 | 业务主表 | 统一保存采购或生产验收单据级事实 |
| 23 | `inspection_order_items` | 质量验收单明细表 | 明细表 | 保存验收 SKU 行级结果 |
| 24 | `inbound_orders` | 入库单主表 | 业务主表 | 统一保存批准的库存增加单据 |
| 25 | `inbound_order_items` | 入库单明细表 | 明细表 | 保存入库 SKU 行级事实 |
| 26 | `outbound_orders` | 出库单主表 | 业务主表 | 统一保存普通库存减少单据 |
| 27 | `outbound_order_items` | 出库单明细表 | 明细表 | 保存出库 SKU 行级事实 |
| 28 | `sales_returns` | 销售退货单主表 | 业务主表 | 保存销售退货单据级业务事实 |
| 29 | `sales_return_items` | 销售退货单明细表 | 明细表 | 保存销售退货 SKU 行级事实 |
| 30 | `damage_reports` | 报损单主表 | 业务主表 | 保存报损单据级业务事实 |
| 31 | `damage_report_items` | 报损单明细表 | 明细表 | 保存报损 SKU 行级事实 |
...
| 序号 | 表名 | 中文名称 | 表类别 | 用途 |
| --- | --- | --- | --- | --- |
| 32 | `cross_border_shipments` | 跨境发货单主表 | 业务主表 | 保存跨境发货单据级业务事实 |
| 33 | `cross_border_shipment_items` | 跨境发货单明细表 | 明细表 | 保存跨境发货 SKU 行级事实 |
| 34 | `shipment_import_matches` | 发货导入匹配表 | 关联表 | 表达跨境发货与导入任务的多对多匹配 |
| 35 | `inventories` | 当前库存表 | 库存表 | 保存每个 SKU 在每个库存节点的当前余额 |
| 36 | `inventory_transactions` | 库存流水表 | 流水表 | 保存每次库存变化历史及来源追溯 |
| 37 | `transfer_orders` | 调拨单主表 | 业务主表 | 保存调拨单据级业务事实 |
| 38 | `transfer_order_items` | 调拨单明细表 | 明细表 | 保存调拨 SKU 行级事实 |
| 39 | `stock_counts` | 盘点单主表 | 业务主表 | 保存盘点单据级业务事实 |
| 40 | `stock_count_items` | 盘点明细表 | 明细表 | 保存 SKU 实盘与差异行级事实 |
| 41 | `inventory_adjustments` | 库存调整单主表 | 业务主表 | 保存库存调整单据级业务事实 |
| 42 | `inventory_adjustment_items` | 库存调整单明细表 | 明细表 | 保存库存调整 SKU 行级事实 |
| 43 | `inventory_alerts` | 库存预警表 | 业务记录表 | 保存库存预警生成、处理和关闭历史 |
...
| 序号 | 表名 | 中文名称 | 表类别 | 用途 |
| --- | --- | --- | --- | --- |
| 44 | `import_tasks` | 导入任务主表 | 任务表 | 统一保存各类批准导入任务 |
| 45 | `import_task_items` | 导入任务明细表 | 明细表 | 保存逐行校验、匹配和执行结果 |
| 46 | `users` | 用户表 | 系统主数据表 | 管理内部用户 |
| 47 | `roles` | 角色表 | 系统主数据表 | 管理角色 |
| 48 | `permissions` | 权限表 | 系统主数据表 | 管理功能及操作权限 |
| 49 | `user_roles` | 用户角色关联表 | 关联表 | 表达用户与角色多对多关系 |
| 50 | `role_permissions` | 角色权限关联表 | 关联表 | 表达角色与权限多对多关系 |
| 51 | `system_settings` | 系统参数表 | 配置表 | 管理系统通用配置 |
| 52 | `backup_tasks` | 数据备份任务表 | 任务表 | 保存备份执行及清理历史 |
| 53 | `attachments` | 附件表 | 资源表 | 统一管理附件基础记录 |
| 54 | `attachment_links` | 附件关联表 | 关联表 | 统一表达附件与受控业务对象关系 |
| 55 | `audit_logs` | 统一审计日志表 | 日志表 | 保存关键操作和系统重要事件 |
| 56 | `document_status_histories` | 单据状态历史表 | 历史记录表 | 保存单据状态变化历史 |
| 57 | `approval_records` | 审批记录表 | 历史记录表 | 保存审核、反审核和审批结果 |
...
22. `inspection_orders`
23. `inspection_order_items`
24. `inbound_orders`
25. `inbound_order_items`
26. `outbound_orders`
27. `outbound_order_items`
28. `sales_returns`
29. `sales_return_items`
30. `damage_reports`
31. `damage_report_items`

### 9.4 跨境与库存类（12 张）

32. `cross_border_shipments`
33. `cross_border_shipment_items`
34. `shipment_import_matches`
35. `inventories`
36. `inventory_transactions`
37. `transfer_orders`
38. `transfer_order_items`
39. `stock_counts`
40. `stock_count_items`
41. `inventory_adjustments`
42. `inventory_adjustment_items`
43. `inventory_alerts`

### 9.5 系统与治理类（14 张）

44. `import_tasks`
45. `import_task_items`
46. `users`
47. `roles`
48. `permissions`
49. `user_roles`
...
54. `attachment_links`
55. `audit_logs`
56. `document_status_histories`
57. `approval_records`

正式逻辑表总数：57 张。

分类数量校验：11 + 10 + 10 + 12 + 14 = 57。

## 10. 候选表

以下表暂列候选，不进入 57 张正式逻辑表清单：

| 候选表 | 暂不纳入原因 |
| --- | --- |
| `role_warehouses` | 角色仓库数据范围需要结合后续权限字段设计确认 |
| `role_stores` | 角色店铺数据范围需要结合后续权限字段设计确认 |
| `safety_stock_rules` | 安全库存需要确认是 SKU 级配置还是 SKU 与 Warehouse 组合级配置 |

Task 3.3 不得直接将候选表转为正式表。
...
- 字段、字段类型、主外键、索引、SQL、ORM 和数据库技术选型：Not Started；
- 技术开发：Not Started。

Task 3.4 尚未启动。Task 3.3 验收通过前不得启动 Task 3.4，不得定义字段清单、字段名称、字段类型、主键、外键字段、索引、唯一约束的物理实现、SQL、ORM、数据库技术选型、数据库 Schema、物理 ER 模型、页面、API 或业务代码。
