...
- 主键、唯一约束、外键策略、索引和 Check 约束：尚未开始（Not Started）

Phase 1 已正式关闭，Phase 2 已完成并获得批准。Phase 3 Database Design 正在进行，Task 3.1 至 Task 3.4 均已完成并获得批准，Task 3.5 已启动。Task 3.5.1 字段数据类型规范已完成并获得批准，建立数据库无关的逻辑类型、长度与精度标准。下一小任务为 Task 3.5.2 主键与唯一约束设计，当前尚未启动；验收 Task 3.5.1 前不得开始 Task 3.5.2。具体主键和唯一约束清单、外键策略、索引、Check 约束、SQL、ORM、Schema、Migration 和技术开发均未开始。
