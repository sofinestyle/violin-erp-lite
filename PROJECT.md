...
- 外键关系规范、普通查询索引和 Check 约束：尚未开始（Not Started）

Phase 1 已正式关闭，Phase 2 已完成并获得批准。Phase 3 Database Design 正在进行，Task 3.1 至 Task 3.4 均已完成并获得批准，Task 3.5 已启动。Task 3.5.1 字段数据类型规范和 Task 3.5.2 主键与唯一约束设计均已完成并获得批准；全部 60 张正式表统一使用单字段 UUID 主键 `id`，业务唯一性通过独立唯一约束表达。下一小任务为 Task 3.5.3 外键关系规范，当前尚未启动；验收 Task 3.5.2 前不得开始 Task 3.5.3。外键关系规范、普通查询索引、Check 约束、SQL、ORM、Schema、Migration 和技术开发均未开始。
