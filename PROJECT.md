...
- Phase 7 Final Consistency Review：Waiting / Not Started
- 业务需求分析：Approved
- 业务规则：Frozen
- 项目治理文档：Approved
- 九阶段开发路线：Frozen
...
Phase 1 已正式关闭，Phase 2 已完成并获得批准。Phase 3 Database Design 及 Task 3.1 至 Task 3.5.7 已完成并获得批准，Database Logical Design v1.1 已冻结；DCR-001 仅批准既有 `completion_status` 的正式状态定义，数据库结构不变。Phase 4 页面设计及 Task 4.1 至 Task 4.10 均为 Completed / Approved。API Change Request 001 已获得批准，库存盘点、销售退货和报损共 43 个接口已纳入 API Master Specification v1.1；API Coverage Completion 002 已获得批准，用户、角色与权限管理共 16 个接口已纳入同一规范；API Coverage Completion 003 已获得批准，角色仓库与店铺数据范围维护共 4 个接口已纳入同一规范；正式接口总数为 335，v1.1 状态为 Completed / Approved / Frozen。Phase 6 功能详细设计及 Final Consistency Review 已完成验收并获得批准，Phase 6 状态为 Completed / Approved / Frozen；Task 6.1 至 Task 6.3 均为后续技术阶段的正式产品输入。Phase 7 保持 In Progress，Task 7.1 至 Task 7.5 已完成并获得批准，Task 7.6 系统集成与开发收口已正式启动，Phase 7 Final Consistency Review 尚未启动。
...
| Phase 3 | 数据库设计 | Database Design | Completed / Approved / Frozen |
...
| Phase 7 | 开发规范与 Codex 执行 | Development Specification | In Progress |
...
当前 Phase 与 Task 状态以 `docs/00-governance/CURRENT_STATUS.md` 为唯一入口，并由 `pnpm status:check` 核对 `ROADMAP.md`、`PROJECT.md` 和 `README.md` 的摘要一致性。
